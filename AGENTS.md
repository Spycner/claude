...
| `workbench`               | 0.22.0  | `brainstorming`, `writing-spec`, `writing-plans`, `visualizing-options`, `using-workbench`, `terse-mode`, `autopilot`, `copilot`, `verification-before-completion`, `test-driven-development`, `dispatching-parallel-agents`, `subagent-driven-development`, `systematic-debugging`, `crafting-html`, `crafting-design-systems` |
| `terminal`                | 0.1.0   | `tmux`                                                                                                                                                                                                                                                                                                                          |
...
| `deprecated`              | 0.1.0   | Frozen archive: `crafting-presentations`, `perfecting-presentations`, `exporting-decks-to-pptx` (formerly `workbench`), `presentations` (formerly `writing`); each points at its `presentations` replacement                                                                                                                    |
...
- Prefer invoking the CLI or `curl` directly in the skill (no shipped wrapper scripts; a skill may direct the agent to write a one-off script at runtime, see the "No wrapper scripts" Design Decision)
...
- For negative routing (skill MUST NOT trigger for a given prompt), use `--not`: `PLUGIN_DIR=plugins/<plugin> bash tests/skill-triggering/run-test.sh --not <skill-name> tests/skill-triggering/prompts/<file>.txt`. Use this when two skills sit close enough that a prompt could plausibly route either way (e.g. `writing:writing --format talk` vs `presentations:designing-presentations`, where a "write me a talk" prompt should land on the prose pipeline not the deck pipeline).
