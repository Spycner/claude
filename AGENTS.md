...
| `writing`                 | 1.7.1   | `writing`, `pyramid`, `tech-doc`, `presentations`                                                                                                                                                                                                                                                                                                                                                                |
| `runtime-bridge`          | 0.1.1   | `claude-codex-bridge`                                                                                                                                                                                                                                                                                                                                                                                            |
| `agent-system-management` | 0.5.1   | `improving-instructions`, `capturing-session-learnings`, `creating-skills`                                                                                                                                                                                                                                                                                                                                       |
| `workbench`               | 0.21.1  | `brainstorming`, `writing-spec`, `writing-plans`, `visualizing-options`, `using-workbench`, `terse-mode`, `autopilot`, `copilot`, `verification-before-completion`, `test-driven-development`, `dispatching-parallel-agents`, `subagent-driven-development`, `systematic-debugging`, `crafting-html`, `crafting-design-systems`, `crafting-presentations`, `exporting-decks-to-pptx`, `perfecting-presentations` |
| `terminal`                | 0.1.0   | `tmux`                                                                                                                                                                                                                                                                                                                                                                                                           |
...
- **Artifact exporters are consumers, not producers.** `workbench:exporting-decks-to-pptx` is the first skill whose input is another skill's finished artifact (a `crafting-presentations` deck) and whose output is a non-HTML format (`.pptx`). Exporters deliberately skip the HTML-producer scaffolding: no `## Output Format` section, no `.workbench/config.md` consultation, and no entry in `crafting-design-systems`' CSS variable inventory, because an exporter reads design tokens from the artifact itself (the deck's shared CSS, then its inline `<style>` overrides, which win) rather than from the design-system layer. Output path conventions live in the exporter's own SKILL.md. Producer and exporter cross-reference each other in one sentence each, and the exporter's frontmatter description names the producer so routing between "build a deck" and "export this deck" stays unambiguous. When adding a future exporter (for example spec-to-docx or report-to-pdf), mirror this consumer shape rather than the producer shape.
