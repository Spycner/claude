...
execute-handler source extracted in .workbench/tool_handlers.json.
...
HANDLERS = REPO / ".workbench" / "tool_handlers.json"
...
def render_handler_notes(tool: str, h: dict) -> str:
    ...
    if truncated:
        ...
        lines.append(
            ...
            "is in `.workbench/tool_handlers.json` under "
            f"`{tool}.body` and the captured factory in "
            f"`.workbench/tool_factories.json`.)_"
        )
    lines.append("")
