# Genie Code skill: `calculation`

Full content of the built-in Genie Code skill `calculation`, extracted from the Databricks SPA webpack chunks on workspace `dbc-df321db9-486f.cloud.databricks.com` (captured 2026-05-20). Loaded by the **Dashboard Authoring Agent** (`dashboardAuthoringAgent`) — see [`../../agents/dashboard-authoring.md`](../../agents/dashboard-authoring.md). The agent loads this file via the `readSkillFile` tool when the task involves custom calculations on dataset measures or dimensions.

## Registry record

Defined as `aV` in chunk `40961.110e90c1b8.chunk.js` (module `229581` — `DashboardAuthoringAgentContext`). Referenced from the agent factory as `skillsets:[{name:"dashboardAgent",skills:[aW,aV,...]}]`.

```jsonc
{
  "name":        "calculation",
  "description": "Custom calculations in AI/BI dashboards. Use when user needs to create calculated measures, calculated dimensions, or use aggregate functions without modifying dataset queries.",
...
The file lives in its own webpack chunk and is exposed as a raw string `module.exports`, loaded on demand via `r.e(<chunk>).then(r.t.bind(r, <module>, 17))`. Chunk hashes change per release — re-derive them from the asset manifest before re-extracting. The path the agent uses with `readSkillFile` is `skills/calculation/SKILL.md`.

| File | Chunk id | Module id | Feature flag |
|---|---|---|---|
| `SKILL.md` | `50147` (hash `54b3f6ca2e` at capture) | `972528` | — |

## Loaded by

| Agent | How it's loaded |
|---|---|
| Dashboard Authoring Agent (`dashboardAuthoringAgent`) | Static — second entry in the `dashboardAgent` skillset. Always present. |

Maps to the `editDatasetCalculations` tool — see [`../../tools/dashboard/editDatasetCalculations.md`](../../tools/dashboard/editDatasetCalculations.md). The system prompt's **Building Widgets: Where to Express Each Metric** section routes any "compound" metric (ratio, percentage, multi-aggregate combination) through `editDatasetCalculations`, and this skill is the canonical reference for what expressions that tool accepts.

## Files

### `SKILL.md`

Webpack chunk `50147.54b3f6ca2e.chunk.js`, module `972528`. 7,229 bytes.
