...
_(body truncated at 2,200 chars; full 2578-byte source is in `../../extraction/data/tool-handlers.json` under `askDataroom.body` and the captured factory in `../../extraction/data/tool-factories.json`.)_
...
Captured 2026-05-19 by patching `window.fetch` + `XMLHttpRequest` on the SPA and triggering the prompt below. Full request log (with bodies) is in [../../traces/askDataroom.json](../../traces/askDataroom.json).

**Trigger prompt:** `use a genie space to find the largest table in samples.nyctaxi`

**Outcome:** could not trigger on this workspace.

No Genie Spaces exist on this workspace (zero hits from `recommend-spaces`). The agent fell back to recommending adjacent assets instead of firing `askDataroom`. To verify this tool, create a Genie Space (`Genie Spaces > New space`) over any UC table, then prompt: `use my <name> genie space to find the largest table`. The expected endpoint is `POST /ajax-api/2.0/data-rooms/<space-id>/ask` — captured here as the only candidate from the handler's chunk source but not yet wire-confirmed.
