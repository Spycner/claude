...
_(body truncated at 2,200 chars; full 2365-byte source is in `../../extraction/data/tool-handlers.json` under `runDatabricksCli.body` and the captured factory in `../../extraction/data/tool-factories.json`.)_
...
Captured 2026-05-19 by patching `window.fetch` + `XMLHttpRequest` on the SPA and triggering the prompt below. Full request log (with bodies) is in [../../traces/runDatabricksCli.json](../../traces/runDatabricksCli.json).

**Trigger prompt:** `run `databricks current-user me` for me`

**Endpoints fired (excluding agent-housekeeping common to every turn):**

- `POST /notebook/<thread-id>/command/<command-id>`
- `GET /ajax-api/2.1/clusters/get`
- `GET /notebook/<thread-id>/has-execution-context`

The CLI command is executed inside the thread notebook via `%sh databricks <args>` (so the actual transport is the same notebook command POST used by `executeCode`). `clusters/get` is queried beforehand to pick a compute target; `has-execution-context` polls for cluster attachment. There is no separate CLI proxy endpoint — the CLI runs on whatever notebook compute the user is attached to.
