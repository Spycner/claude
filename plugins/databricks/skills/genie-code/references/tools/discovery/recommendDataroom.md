...
Captured 2026-05-19 by patching `window.fetch` + `XMLHttpRequest` on the SPA and triggering the prompt below. Full request log (with bodies) is in [../../traces/recommendDataroom.json](../../traces/recommendDataroom.json).

**Trigger prompt:** `what genie spaces do I have on this workspace?`

**Endpoints fired (excluding agent-housekeeping common to every turn):**

- `POST /ajax-api/2.0/data-rooms/tools/recommend-spaces`
- `POST /graphql/UnifiedAssetSearch`
- `POST /graphql/AssistantListFavoriteAssets`
- `POST /graphql/AssistantListPopularAssets`
- `POST /graphql/AssistantListSuggestedAssets`

The dedicated handler is `POST /ajax-api/2.0/data-rooms/tools/recommend-spaces` — this is the ground-truth endpoint that returns the candidate Genie Spaces. It runs alongside the four assistant-asset listings (`UnifiedAssetSearch` + `AssistantList{Favorite,Popular,Suggested}Assets`) so the agent can mention adjacent dashboards / queries when no relevant space exists. On this workspace zero spaces returned, and the agent gracefully degraded to the alternatives.
