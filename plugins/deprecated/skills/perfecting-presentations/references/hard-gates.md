...
Deterministic probes for the H rules. Run them with any driveable Chrome (the chrome-devtools MCP tools, the chrome-devtools-cli skill, or a small Puppeteer script); the probes themselves are plain JavaScript evaluated in the page. Every failure here is a confirmed fix item with no judging step.
...
```bash
python3 -m http.server 8123 --directory <deck-dir> &
```
...
- H4: read the console log (`list_console_messages` on MCP); any `error`-level entry fails unless its resource is on the constitution's H4 allowlist (favicons, optional state probes some deck engines fire).
...
Capture after the probe, with `noscale` removed and the thumbnail rail suppressed so judges see what the audience sees. On MCP: resize to 1920x1080 (`resize_page`), then for each slide navigate to `<url>#N` (1-based), reload if the hash change does not repaint, wait for the slide to be active, and `take_screenshot` to `.workbench/reviews/<deck-slug>/round-<R>/slide-NN.png`. The deck hides its overlay chrome after about two seconds of mouse idle; avoid moving the mouse between navigate and capture.
...
Collect all gate results into `.workbench/reviews/<deck-slug>/round-<R>/hard-gates.json`: one entry per issue with gate, slide, and evidence string, plus a `pass` boolean per gate. Hard-gate failures skip review and verification; they go straight onto the fix list.
