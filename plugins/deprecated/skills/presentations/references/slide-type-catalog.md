...
The eight content-side slide types used by `deprecated:presentations`. Each type is a role in the argument, not a visual template. Rendering happens later in `deprecated:crafting-presentations`; see the mapping at the bottom of this file.
...
## Rendering hand-off to deprecated:crafting-presentations

The content-side type names above describe the role each slide plays in the argument. `deprecated:crafting-presentations` works in a different vocabulary: it names rendering templates (visual layouts with slots for headline, body, image, chart, etc.). The recommended mapping below lets a host agent pass a content-side deck plan to `deprecated:crafting-presentations` without re-deriving the layout from scratch each time.

| Content-side type (this skill) | Recommended rendering template (`deprecated:crafting-presentations`) |
| ------------------------------ | -------------------------------------------------------------------- |
| Title                          | Title                                                                |
| Agenda                         | AgendaSlide                                                          |
...
The mapping is a recommendation, not a constraint. `deprecated:crafting-presentations` remains free to pick a different rendering when the slide's visual brief calls for it (a Decision slide whose proof is a photograph rather than a chart may render better as an ImageSlide with a decision callout, for example). The two taxonomies sit at different levels of abstraction: this skill names the slide's role in the argument (content-role), while `deprecated:crafting-presentations` names the layout that paints it (render-template). A one-to-one mapping is convenient but not load-bearing.
