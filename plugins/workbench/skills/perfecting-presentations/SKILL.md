...
- `workbench:crafting-presentations` renders the deck this skill perfects and documents the `<deck-stage>` engine and slide-type catalog.
- `writing:presentations` designs the content; if review keeps finding narrative blockers, the deck likely needs a content pass there, not more polish rounds.
