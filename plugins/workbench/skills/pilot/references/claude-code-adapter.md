# Claude Code Adapter for Workbench Pilot

This doc maps the pilot skill's tool references to Claude Code tool names and documents Claude-Code-specific patterns. Read alongside `SKILL.md`.
...
Slice tasks dispatch through `Agent` with `subagent_type=general-purpose`. Three regimes:

### Independent tasks (parallel)

No shared files, no ordering dependency. Confirm the tasks have disjoint write scopes (see `workbench:subagent-driven-development`'s parallel dispatch guidance), then send multiple `Agent` calls in a single message; they run in parallel.
...
- The slice it owns. Paste the relevant checkbox block from the plan.
...
When the `slices` gate is active, pause after each slice's review gates pass: present the diff summary and end the turn so the user can respond. Do not simulate the user's approve or resteer decision and continue in the same turn; a real pause means the assistant message ends and the next slice does not start until the user's next message arrives.
...
- **`gh` auth.** Assume `gh` is already authenticated. If it is not, surface that to the user instead of trying to authenticate inside pilot.
