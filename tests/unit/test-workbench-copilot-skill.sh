...
# Test 10: Plugin manifests and marketplace at 0.17.0
echo "Test 10: Plugin manifests and marketplace at 0.17.0..."
...
if jq -e '.version == "0.17.0"' "$CCM" >/dev/null && jq -e '.version == "0.17.0"' "$CXM" >/dev/null \
   && jq -e '.plugins[] | select(.name == "workbench") | .version == "0.17.0"' "$MP" >/dev/null; then
    echo "  [PASS] manifests and marketplace at 0.17.0"
else
    echo "  [FAIL] workbench not at 0.17.0"; exit 1
fi
echo ""
